...
### Font Family
- **Primary Font**: `Paperlogy` — 이 프로젝트의 **무조건적인 표준 폰트**입니다. 한글, 영문, 숫자가 최적화된 조합형 폰트입니다.
- **CDN**: `@import url('https://cdn.jsdelivr.net/gh/fonts-archive/Paperlogy/Paperlogy.css');`
- **Fallbacks**: `sans-serif`

### Hierarchy (Paperlogy 기준)

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.hero-display}` | 56px | 800 (Bold) | 1.07 | -0.04em | Hero headline; "Apple tight" 감성 구현 |
| `{typography.display-lg}` | 40px | 700 | 1.10 | -0.03em | Tile headlines |
| `{typography.display-md}` | 34px | 700 | 1.47 | -0.03em | Section heads |
| `{typography.lead}` | 28px | 400 | 1.14 | 0 | Product tile subcopy |
| `{typography.body}` | 17px | 400 | 1.47 | -0.01em | Default paragraph (17px 유지) |
| `{typography.caption}` | 14px | 400 | 1.43 | 0 | Secondary captions, button text |

### Principles

- **Paperlogy Tight**: 한글 폰트 특성에 맞춰 대형 제목은 반드시 `letter-spacing: -0.03em ~ -0.05em`을 적용하여 밀도 있는 레이아웃을 구성합니다.
- **Weight Strategy**: Paperlogy는 1(Thin)부터 9(Black)까지 지원하므로, 헤드라인은 7~8, 본문은 4~5를 주로 사용합니다.
- **Body size 17px**: Apple의 유산을 계승하여 본문 크기는 반드시 17px를 유지합니다.
